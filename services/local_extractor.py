...
def detect_bank(pdf_path):
    ...
    if "royalbank" in text_norm or "rbc" in text_norm:
        return "RBC"
    if "tdcanadatrust" in text_norm or "tdbank" in text_norm:
        return "TD"
    if "bankofmontreal" in text_norm or "bmo" in text_norm:
        return "BMO"
    if "cibc" in text_norm:
        return "CIBC"
    if "tangerine" in text_norm:
        return "Tangerine"
    if "vancity" in text_norm or "vancouvercitysavings" in text_norm:
        return "Vancity"
...
def extract_vancity_pdf(pdf_path):
    """
    Specialized parser for Vancity Credit Union statements to handle spaced-out text layout.
    """
    all_lines = []
    try:
        if hasattr(pdf_path, "seek"):
            ...
            for page in reader.pages:
                ...
                if txt:
                    all_lines.extend([line.strip() for line in txt.split("\n") if line.strip()])
        else:
            with open(pdf_path, "rb") as f:
                reader = pypdf.PdfReader(f)
                for page in reader.pages:
        ...
        print(f"Error reading Vancity PDF lines: {e}")
        return [], 0.0
    ...
    date_re = re.compile(
        r'\b([0-3]?\s*[0-9])\s*(j\s*a\s*n|f\s*e\s*b|m\s*a\s*r|a\s*p\s*r|m\s*a\s*y|j\s*u\s*n|j\s*u\s*l|a\s*u\s*g|s\s*e\s*p|o\s*c\s*t|n\s*o\s*v|d\s*e\s*c)\b',
        re.IGNORECASE
    )

    double_num_re = re.compile(
        r'(\-?\s*\d[\d\s,]*\.\s*\d\s*\d)\s+(\-?\s*\d[\d\s,]*\.\s*\d\s*\d)\s*$'
    )
    single_num_re = re.compile(
        r'(\-?\s*\d[\d\s,]*\.\s*\d\s*\d)\s*$'
    )

    def clean_spaced_word(text):
        ...
        current = ""
        for t in tokens:
            if len(t) <= 2 or t.lower() in ["trf", "fee"]:
                current += t
            else:
                if current:
        ...
        cleaned = " ".join(merged)
        cleaned = cleaned.replace("PRE AU THORIZED", "PREAUTHORIZED")
        cleaned = cleaned.replace("ETRA NS FER", "ETRANSFER")
        cleaned = cleaned.replace("D EB IT", "DEBIT")
        cleaned = cleaned.replace("CRE DIT", "CREDIT")
        cleaned = cleaned.replace("UB ER", "UBER")
        cleaned = cleaned.replace("HO LD INGS", "HOLDINGS")
        cleaned = cleaned.replace("CA NAD A", "CANADA")
        cleaned = cleaned.replace("I NC", "INC")
        cleaned = cleaned.replace("T O", "TO")
        cleaned = cleaned.replace("RE L", "REL")
        cleaned = cleaned.replace("P AP E R", "PAPER")
        cleaned = cleaned.replace("MA Y2", "MAY 2")
        return cleaned
    ...
    for line in all_lines:
        ...
        if "opening balance" in line_lower:
            s_match = single_num_re.search(line)
            if s_match:
                try:
                    opening_bal = float(s_match.group(1).replace(" ", "").replace(",", ""))
                except:
                    pass
            break
    ...
    while idx < len(all_lines):
        ...
        line_lower = line.lower()
        if any(h in line_lower for h in ["summary", "patronage", "opening balance", "closing balance", "page total", "subtotal"]):
            idx += 1
            continue
        ...
        if date_match:
            ...
            month = date_match.group(2).replace(" ", "").upper()
            
            # Map month name to digits
            months_map = {
                "JAN": 1, "FEB": 2, "MAR": 3, "APR": 4, "MAY": 5, "JUN": 6,
                "JUL": 7, "AUG": 8, "SEP": 9, "OCT": 10, "NOV": 11, "DEC": 12
            }
            month_num = months_map.get(month[:3], 1)
            day_num = int(day)
            date_str = f"{statement_year}-{month_num:02d}-{day_num:02d}"
            
            remainder = line.replace(date_match.group(0), "").strip()
            
            d_match = double_num_re.search(remainder)
            s_match = single_num_re.search(remainder)
            
            if d_match:
                amt_str = d_match.group(1).replace(" ", "").replace(",", "")
                bal_str = d_match.group(2).replace(" ", "").replace(",", "")
                desc_raw = remainder[:d_match.start()].strip()
                desc = clean_spaced_word(desc_raw)
                ...
                if any(k in desc_lower for k in ["fee", "debit", "trf to", "withdrawal", "payment"]):
                    withdrawal = amt
                else:
                    deposit = amt
                ...
                bal_str = s_match.group(1).replace(" ", "").replace(",", "")
                desc_raw = remainder[:s_match.start()].strip()
                desc = clean_spaced_word(desc_raw)
                ...
                try:
                    ...
                    desc = clean_spaced_word(next_line)
                    
                    next2_line = all_lines[idx+2].strip()
                    amt_match = single_num_re.search(next2_line)
                    amt = float(amt_match.group(1).replace(" ", "").replace(",", "")) if amt_match else 0.0
                    
                    next3_line = all_lines[idx+3].strip()
                    bal_match = single_num_re.search(next3_line)
                    bal = float(bal_match.group(1).replace(" ", "").replace(",", "")) if bal_match else 0.0
                    ...
                    print(f"Failed Vancity lookahead: {e}")
        idx += 1
...
def extract_digital_pdf(pdf_path, bank_name):
    ...
    """
    if bank_name == "Vancity":
        return extract_vancity_pdf(pdf_path)
